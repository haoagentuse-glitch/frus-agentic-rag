...
cmd_launch() {
  ...
  gcloud compute instances create "$VM" \
    ...
    "${spot_args[@]}"
  echo
  echo "VM $VM creating. It provisions itself, runs, uploads to $BUCKET/reports/, then deletes itself."
...
cmd_logs() {
  echo "-- serial console (provisioning) --"
  gcloud compute instances get-serial-port-output "$VM" --zone "$ZONE" --project "$PROJECT" 2>/dev/null | tail -30 || true
