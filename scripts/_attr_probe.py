...
async def main():
    ...
    print()
    print(
        f"最佳段落屬於 gold 文件 : n={len(best_gold):3d} 中位 {q(best_gold, 0.5):7.3f}"
        if best_gold
        else "no gold"
    )
    print(
        f"最佳段落非 gold        : n={len(best_other):3d} 中位 {q(best_other, 0.5):7.3f}"
        if best_other
        else "no other"
    )
    print()
