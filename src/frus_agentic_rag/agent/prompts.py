...
規則：
- 把答案拆成若干條主張，每條是一個可被單一段落佐證的完整句子。引用由程式產生，不要自己寫編號。
...
def synth_user(question: str, evidence: list[Evidence]) -> str:
    ...
    return (
        ...
        "answer_text has to appear as a claim. Leave evidence_ids empty — the "
        "cross-encoder attaches them afterwards, which is why nothing here asks "
        "you to copy an identifier."
    )
