...
import asyncio, json
from frus_agentic_rag.retrieval.hybrid import hybrid_search_sync, bm25_search, dense_search
...
s = get_settings(); f = SearchFilters(subtypes=["historical-document"])
cases = [json.loads(l) for l in open("eval/gold_cases.jsonl") if json.loads(l)["answerable"]]

async def main():
    c = get_client()
    raw_h = tr_h = en_h = T = 0
    for case in cases:
        gold = set(case["gold_documents"]); zh = case["question_zh"]
        ...
        en = {f"{h.volume_id}:{h.document_id}" for h in hybrid_search_sync(case["question_en"], f, 10)}
        raw_h += len(gold & raw); tr_h += len(gold & tr); en_h += len(gold & en); T += len(gold)
    print(f"gold docs = {T}")
