...
完整 ablation 每個 variant 約兩小時，其中大部分沒有買到任何資訊：
**70 個 (case, language) 中有 28 個在全部 20 種配置下分數完全相同**，
它們對每個平均值的貢獻一樣，永遠無法區分任何東西——那 40% 是純成本。

`scripts/select_core_cases.py` 從已記錄的 `reports/runs_*.jsonl` 建立
case × configuration 矩陣，先丟掉零變異的，再貪婪挑選能重現全集
各配置平均值的子集，並釘住各題型與雙語各一題（gate 3 定義在 unanswerable 上，
沒有那類題就無法評估）。

**保真度曲線**（在 20 種配置上量測）：

| 題數 | 加速 | 最大誤差 | 全部配對排序一致 | 差距 >3pp 的配對一致 |
|---:|---:|---:|---:|---:|
| 6 | 5.83× | 17.01pp | 0.621 | 0.673 |
| 8 | 4.38× | 13.36pp | 0.716 | 0.812 |
| **10** | **3.50×** | 7.29pp | 0.868 | **1.000** |
| 12 | 2.92× | 5.05pp | 0.826 | 0.970 |
| 18 | 1.94× | 4.96pp | 0.858 | 1.000 |

**10 題是轉折點**：全集認定為決定性（>3pp）的配對差異，子集 100% 保留。
低於 8 題就崩掉（6 題時只剩 0.673、誤差 17pp）。
...
**兩個必須知道的限制。** 一、子集是在這 20 種配置上選出來的，屬於樣本內驗證；
一個行為模式不同的新配置未必被涵蓋。二、絕對數值會偏移最多 7.29pp——
**它能告訴你哪個設定比較好，不能拿來報告數字**。所以：迭代用子集，
最終結論一定要跑完整 35 題。
...
### 1.1 中文問題有約 40% 拒答，成因是引用編號抄寫失敗 —— **已修**
...
**修法：把編號從生成任務裡拿掉。**（`agent/attribute.py`，新增）

模型只寫主張，不寫編號；由已經在排序的交叉編碼器把每條主張對每份已接受段落評分，
再由程式掛上編號。模型從此**不可能引用到沒被檢索的東西，因為它根本不指名任何東西**。
`config.citation_mode` 預設 `post_hoc`，設為 `model` 可切回舊行為做對照。
2-step 基線也套用同一機制，否則消融會變成在比引用方案而不是比圖。

門檻同樣採相對制（`attribution_margin = 2.0`），理由與證據篩選一致：分數尺度隨題型移動。
`attribution_min_score = -6.0` 是**暫定值**——（主張, 段落）的分數分佈還沒像
（問題, 段落）那樣量過，這是下一個該補的量測。

**引用閘門改為部分接受。** 原本 8 條主張裡 1 條編號壞掉就整篇作廢，
其餘 7 條有效引用一起丟。現在壞的那條被丟掉，答案照出；只有在
**每一條主張都無法引用**時才整篇拒答。無法引用的主張會從答案中移除，
不會留下沒有引用的句子。個別編號被拒的原因改走 `reasons` 通道，
trace 仍看得到是「編造」還是「未被接受」——這個診斷資訊不能因為改成部分接受就消失。

**實測（真實語料，4/4）：** 先前 4 次 smoke 有 2 次拒答、且完全照語言分（兩題中文都是
`no claims: answer carries no citable statements`）。改完之後 **4/4 全部作答**，
主張全數掛上引用、0 條被丟棄、閘門 0 錯誤：

| 案例 | 語言 | 結果 | 主張保留 / 丟棄 |
|---|---|---|---|
| multihop-10 | 中 | 作答 | 7 / 0 |
| multihop-10 | 英 | 作答 | 6 / 0 |
| lookup-00 | 中 | 作答 | 3 / 0 |
| lookup-00 | 英 | 作答 | 3 / 0 |

**一個必須盯的副作用：這個改動移除了一個拒答觸發條件。**
舊制下「模型寫了壞編號」會導致拒答；新制下模型的編號被丟棄，
拒答只剩三條路徑：沒有證據、沒有主張、所有主張的最佳分數都低於
`attribution_min_score`。門檻 3（不可回答題誤答）本來就沒過（2–3 筆），
**下一輪必須確認它沒有變得更糟**。這是用拒答率換正確率的交換，方向要靠量測確認。

### 1.1b 「文件命中」不等於「證據命中」—— 已建立指標，而我的假設被推翻

我先前把「引用到標準文件卻仍被判 0 分」的 15 題歸因於片段切割：
引用單位是文件，模型看到的是 512 詞元的片段，可能引對文件卻沒看到帶事實的那一片。

`scripts/evidence_coverage.py`（新增）把這件事量出來。標準題目集只標文件不標片段，
所以「帶答案的片段」要用推定的：把該文件的每一片都用同一個交叉編碼器對問題評分，
取最高的那片當作證據片段。這是代理指標，而且刻意用系統自己的相關性判準——
因為要問的是「系統自己的排序有沒有把對的片段送到最後」，不是「人類會不會選同一片」。

**在那 15 題上實測（30 份標準文件，平均每份 5.7 片）：**

| 指標 | 數值 |
|---|---:|
| 文件層級召回（進入合成視窗） | 0.9667 |
| **證據片段層級召回（進入合成視窗）** | **0.9667** |
| 文件命中但證據片段沒進來 | **0** |

**假設被推翻。** 文件確實是多片的（平均 5.7 片），但最該進來的那一片，
在 30 份中有 29 份都進到了合成視窗。**片段切割不是這 15 題的成因。**

指標仍然值得保留——它現在是一個被量測過的不變量，而不是一個假設。
但診斷方向要改：既然證據送到了模型面前而答案仍被判錯，
**剩下的解釋只有跨文件綜合能力與評分嚴格度**。
這 15 題裡有 13 題是多文件推理題，與「多文件正確率 0.318」完全吻合。
...
**完整分佈已量完**（35 cases、1400 rows、119 gold，GCP L4，`reports/score_distribution.json`）。

先前根據 5 cases 讀出的「地板 −1.96 可保 100% gold 且只留 27.5%」是錯的，樣本太小。
真實數字是**地板 −4.46、留下 74.8%**——絕對門檻幾乎不篩掉任何東西。分位數顯示原因：
gold 的 p5 是 −3.17，非 gold 的 p95 是 +0.45，兩者尾巴大幅重疊。

| 切片 | gold p50 | 非 gold p50 | 保 100% gold 的地板 | 該地板留下 |
|---|---:|---:|---:|---:|
| 整體 | +0.08 | −3.01 | −4.46 | 74.8% |
| lookup | **+1.86** | −0.75 | −0.57 | 51.5% |
| multihop | **−1.32** | −4.03 | −4.43 | 66.7% |
| correction_matched | −0.83 | −3.46 | −4.46 | 73.0% |

**絕對門檻不要用**，維持 `score_keep_absolute = None`。

**相對 margin 則可用，而且穩定。** `margin = top_score − 最深一份 gold 的分數`
就是這個參數的分佈本身：

| | p50 | p75 | p90 | p95 | max |
|---|---:|---:|---:|---:|---:|
| 全體（47 題有 gold） | 0.72 | 2.08 | 3.50 | 3.96 | **4.28** |

各題型的 max 分別是 lookup 2.86、correction 4.05、multihop 4.28——**跨題型只差 1.4 個單位，
而絕對分數差了 3.2 個單位**。這證實了 margin 會自動隨題型平移，是正確的控制項。
故設 `score_keep_margin = 4.3`（保住全部 gold），或 3.5（保住 90% 的題目）。

**第一份 gold 的 rank**：p50=1、p75=1、p90=4、p95=7、最差 15。
所以 `score_min_per_hop = 3` 覆蓋 75% 的題目，要覆蓋 p95 需要 7。

**recall@k 幾乎是平的，這對策略選擇很重要：**

| k | 1 | 2 | 3 | 5 | 10 | 20 |
|---|---:|---:|---:|---:|---:|---:|
| recall | 0.557 | 0.586 | 0.600 | 0.629 | 0.657 | 0.671 |

從 1 份到 20 份只多 11.4pp，top-3 就拿到可達上限的 89%。
**這是支持策略 A（固定 top-k）的直接證據**——B/C 的複雜度最多只能買到 7pp。

另外 70 個 (case, language) 組合中有 **23 個在前 20 名裡完全沒有 gold**，
檢索上限因此是 0.671。任何篩選都救不了這部分，那是檢索臂的問題，不是篩選的問題。
